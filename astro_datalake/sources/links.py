...
LINKS["spacetrack"] = SourceLinks(
    ...
    targets=(
        DownloadTarget(
            filename="gp.json",
            url=(
                f"{SPACETRACK_QUERY}/class/gp/decay_date/null-val/epoch/%3Enow-10"
                "/orderby/NORAD_CAT_ID/format/json"
            ),
            timeout=900.0,
            media_type="application/json",
            note=(
                "Current GP element set for on-orbit objects. The predicates are the "
                "ones the API documentation asks for: 'Add /decay_date/null-val/"
                "epoch/%3Enow-10/ to the URL to ensure that you only retrieve "
        ...
        DownloadTarget(
            ...
            media_type="application/json",
            note=(
                "Decay messages. The docs cap this class at one pull per day and ask "
                "that you store what you download rather than re-fetching it; the "
                "downloader enforces both through its retrieval ledger."
            ),
        ),
    ),
    note=(
        "Free account required. The downloader POSTs identity/password to "
        f"{SPACETRACK_LOGIN} to get a session cookie, GETs the query URLs above, then "
        "logs out. Without both env vars set the source is skipped, never "
        "half-attempted. Requests are throttled below the documented ceilings "
        "(<30/minute, <300/hour) and each class is held to its documented retrieval "
        "rate — GP hourly, SATCAT and BOXSCORE daily after 1700 UTC — via an on-disk "
        "ledger. See sources/spacetrack.py."
    ),
)
